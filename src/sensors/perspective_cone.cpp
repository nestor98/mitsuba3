...
.. _sensor-PerspectiveCone:

Irradiance meter (:monosp:`PerspectiveCone`)
--------------------------------------------
...
MI_VARIANT class PerspectiveCone final : public Sensor<Float, Spectrum> {
public:
    MI_IMPORT_BASE(Sensor, m_film, m_shape, sample_wavelengths)
    MI_IMPORT_TYPES(Shape)

    PerspectiveCone(const Properties &props) : Base(props) {
