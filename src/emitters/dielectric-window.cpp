...
This plugin implements an emitter encapsulating a (maybe rough) dielectric separator and another emitter.
This is useful for, e.g., modeling a water surface from the inside, where Snell's law makes single scattering only come from a 
cone directly above any point underwater.  

This means that this plugin needs 3 inner components:
- The separating shape
- The dielectric material
- The inner emitter (e.g., an envmap for the sky)
...
class DielectricWindow final : public Emitter<Float, Spectrum> {
public:
    MI_IMPORT_BASE(Emitter, m_flags, m_medium, m_to_world)
    MI_IMPORT_TYPES(Scene, Texture)

    DielectricWindow(const Properties &props) : Base(props) {
        ...
        for (auto &[name, obj] : props.objects(false)) {
            auto *emitter = dynamic_cast<Base *>(obj.get());
            if (emitter) {
                if (em_index == 1)
                    Throw("DielectricWindow only supports one child emitter!");
                [em_index++] = emitter;
    ...
    std::pair<DirectionSample3f, Spectrum> sample_direction(const Interaction3f &it,
        ...
        ds.delta    = false;
        ds.emitter  = this;
        ds.d        = m_to_world.value() * local_dir;
...
MI_IMPLEMENT_CLASS_VARIANT(DielectricWindow, Emitter)
MI_EXPORT_PLUGIN(DielectricWindow, "Spot emitter")
